      ...
      <div style={{ background: '#1C1916', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '36px 40px 0' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 0, borderTop: '1px solid rgba(255,255,255,0.07)', marginTop: 16 }}>
            {[
              { label: 'Employees on Platform', value: empCount === null ? '…' : empCount, up: false },
              { label: 'Active Adventures',      value: active,    up: true  },
              { label: 'Bookings This Month',    value: thisMonth, up: true  },
              { label: 'Pending Approval',       value: pending,   up: false },
            ].map((s, i) => (
        ...
        <div className="card" style={{ padding: '22px 28px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
            <div>
              ...
              <p style={{ fontSize: 12.5, color: colors.muted }}>Verified marketplace partners · {vendors.length} total</p>
            </div>
            <Button small variant="ghost" onClick={() => navigate('/hr/integrations')}>Manage vendors →</Button>
          </div>
          {vendors.length === 0 ? (
            <p style={{ fontSize: 13, color: colors.muted }}>No vendors yet. Vendors appear here once they register and are verified.</p>
          ) : (
              ...
              {vendors.map(v => (
                <div key={v.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 16px', background: '#F7F5F2', borderRadius: 12, border: '1px solid #eee' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: 11.5, color: colors.muted, textTransform: 'capitalize' }}>{v.category?.replace('_',' ')}</p>
                  </div>
