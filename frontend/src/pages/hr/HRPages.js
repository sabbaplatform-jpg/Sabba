          ...
          <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {['all','pending','approved','confirmed','cancelled'].map(s => (
                <button key={s} onClick={() => setFilter(s)} style={{ padding: '8px 16px', borderRadius: 20, border: `1.5px solid ${filter===s ? colors.orange : '#eee'}`, background: filter===s ? colors.orangeLight : '#fff', color: filter===s ? colors.orange : colors.mid, fontSize: 12.5, fontWeight: 700, cursor: 'pointer', fontFamily: font.body }}>
    ...
    <Modal title="Package Review" onClose={onClose} width={620}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 16 }}>
        {[
          ...
          { label: 'Price',       value: `£${Number(pkg.price_gbp||0).toLocaleString()}` },
          { label: 'Destination', value: pkg.destination },
          { label: 'Vendor',      value: pkg.vendor_name },
          { label: 'Verified',    value: pkg.verified ? '✓ Yes' : 'Not yet' },
        ].map((item, i) => (
      ...
      {pkg.description && (
        <div style={{ background: '#F7F5F2', borderRadius: 10, padding: '14px 16px', marginBottom: 20 }}>
          <p style={{ fontSize: 11, fontWeight: 700, color: colors.faint, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 8 }}>Description</p>
          <p style={{ fontSize: 13.5, color: colors.mid, lineHeight: 1.7 }}>{pkg.description}</p>
        </div>
      )}
      <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
        <Button variant="secondary" onClick={onClose}>Close</Button>
        {adminStatus !== 'rejected' && (
          <button onClick={() => { onReject(pkg.id); onClose(); }} style={{ background: colors.redLight, color: colors.red, border: 'none', borderRadius: 10, padding: '10px 20px', fontSize: 13.5, fontWeight: 700, cursor: 'pointer', fontFamily: font.body }}>Reject</button>
        )}
        {adminStatus !== 'approved' && (
          <button onClick={() => { onApprove(pkg.id); onClose(); }} style={{ background: colors.green, color: '#fff', border: 'none', borderRadius: 10, padding: '10px 20px', fontSize: 13.5, fontWeight: 700, cursor: 'pointer', fontFamily: font.body }}>✓ Approve</button>
        )}
      </div>
    </Modal>
  );
...
export function HRMarketplace() {
  ...
  return (
    <div style={{ fontFamily: font.body, background: '#F7F5F2', minHeight: '100vh', paddingBottom: 80 }}>
      {reviewing && (
        <PackageReviewModal pkg={reviewing} onClose={() => setReviewing(null)}
          onApprove={id => approvePackage(id,'approved')}
          onReject={id => approvePackage(id,'rejected')}
        />
      )}
      <div style={{ background: '#fff', borderBottom: '1px solid #eee', padding: '28px 40px 24px' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          ...
          <p style={{ color: colors.muted, fontSize: 14, fontWeight: 500, marginBottom: 16 }}>Click Review to inspect a package before approving or rejecting it.</p>
                ...
                <div key={pkg.id} className="card" style={{ overflow: 'hidden' }}>
                  <div style={{ height: 160, background: gradient, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                    ...
                    <div style={{ position: 'absolute', top: 12, left: 12 }}>
                      <span style={{ fontSize: 10, fontWeight: 700, color: '#fff', background: 'rgba(0,0,0,0.42)', borderRadius: 6, padding: '4px 8px', textTransform: 'uppercase' }}>{pkg.category?.replace('_',' ')}</span>
                    </div>
                    <div style={{ position: 'absolute', top: 12, right: 12 }}><Badge status={adminStatus}/></div>
                    {pkg.verified && (
                  ...
                  <div style={{ padding: '16px 18px 18px' }}>
                    ...
                    <p style={{ fontSize: 12.5, color: colors.muted, marginBottom: 14, fontWeight: 500 }}>{pkg.vendor_name} · {pkg.destination} · {pkg.duration}</p>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <div style={{ display: 'flex', gap: 6 }}>
                        ...
                        {adminStatus !== 'approved' && (
                          <button onClick={() => approvePackage(pkg.id,'approved')} style={{ background: colors.greenLight, color: colors.green, border: 'none', borderRadius: 8, padding: '6px 12px', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: font.body }}>Approve</button>
                        )}
                        {adminStatus !== 'rejected' && (
                          <button onClick={() => approvePackage(pkg.id,'rejected')} style={{ background: colors.redLight, color: colors.red, border: 'none', borderRadius: 8, padding: '6px 12px', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: font.body }}>Reject</button>
                        )}
                      </div>
                    </div>
                  </div>
...
export function HRAnalytics() {
  ...
  const handleExport = () => {
    ...
    const rows = bookings.map(b => [
      ...
      b.payroll_months||'', Number(b.monthly_amount||0).toFixed(2),
      Number(b.total_amount||0).toFixed(2), b.payment_method||'payroll', b.status||'',
    ]);
    const csv = [headers, ...rows].map(r => r.map(c => `"${String(c).replace(/"/g,'""')}"`).join(',')).join('\n');
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const url  = URL.createObjectURL(blob);
    const a    = document.createElement('a');
    a.href = url; a.download = `sabba-analytics-${new Date().toISOString().slice(0,10)}.csv`;
  ...
  return (
    <div style={{ fontFamily: font.body, background: '#F7F5F2', minHeight: '100vh', paddingBottom: 80 }}>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '28px 40px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16, marginBottom: 24 }}>
          {stats.map((s, i) => (
            <div key={i} className="card" style={{ padding: '20px 22px' }}>
              ...
              <p style={{ fontSize: 12, color: colors.muted, fontWeight: 600 }}>{s.label}</p>
            </div>
          ))}
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
          <div className="card" style={{ padding: '22px 24px' }}>
            <p style={{ fontSize: 15, fontWeight: 700, color: colors.dark, marginBottom: 20 }}>Total Programme Value</p>
            ...
            <div style={{ marginTop: 20, height: 6, background: '#eee', borderRadius: 3, overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${Math.min((confirmed/Math.max(bookings.length,1))*100,100)}%`, background: `linear-gradient(90deg, ${colors.orange}, #f5a066)`, borderRadius: 3 }}/>
            </div>
            <p style={{ fontSize: 12, color: colors.muted, marginTop: 6 }}>{confirmed} of {bookings.length} confirmed</p>
          </div>
          <div className="card" style={{ padding: '22px 24px' }}>
            <p style={{ fontSize: 15, fontWeight: 700, color: colors.dark, marginBottom: 20 }}>Monthly Bookings</p>
              ...
              {byMonth.map((m, i) => {
                ...
                return (
                  <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, height: '100%', justifyContent: 'flex-end' }}>
                    ...
                    <div style={{ width: '100%', borderRadius: '4px 4px 0 0', height: `${Math.max(pct, 4)}%`, background: isLast ? `linear-gradient(to top, ${colors.orange}, #f5a066)` : '#F0EDE9' }}/>
                  </div>
                );
              })}
            </div>
        ...
        <div className="card" style={{ padding: '22px 24px' }}>
          <p style={{ fontSize: 15, fontWeight: 700, color: colors.dark, marginBottom: 20 }}>Bookings by Destination</p>
...
function VendorReviewModal({ vendor, onClose, onVerify, onRevoke }) {
  const pkgCount = vendor.package_count || 0;
  return (
    <Modal title="Vendor Review" onClose={onClose} width={580}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20, padding: 16, background: '#F7F5F2', borderRadius: 12 }}>
        {vendor.avatar_url
          ? <img src={vendor.avatar_url} alt={vendor.company_name} style={{ width: 56, height: 56, borderRadius: 14, objectFit: 'cover' }}/>
          : <div style={{ width: 56, height: 56, borderRadius: 14, background: `linear-gradient(135deg, ${colors.orange}, #f5a066)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24 }}>🏢</div>
        }
        <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>
        {[
          ...
          { label: 'Packages',    value: `${pkgCount} listed` },
          { label: 'Website',     value: vendor.website || 'Not provided' },
          { label: 'Rating',      value: vendor.rating > 0 ? `${vendor.rating} ★` : 'No ratings yet' },
          { label: 'Member since', value: vendor.created_at ? new Date(vendor.created_at).toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'}) : '—' },
          { label: 'Pending since', value: vendor.pending_since ? new Date(vendor.pending_since).toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'}) : '—' },
        ].map((item, i) => (
          <div key={i} style={{ background: '#F7F5F2', borderRadius: 10, padding: '10px 12px' }}>
            <p style={{ fontSize: 10, fontWeight: 700, color: colors.faint, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 3 }}>{item.label}</p>
            <p style={{ fontSize: 13, fontWeight: 600, color: colors.dark }}>{item.value || '—'}</p>
      ...
      {vendor.about && (
        <div style={{ background: '#F7F5F2', borderRadius: 10, padding: '14px 16px', marginBottom: 16 }}>
          <p style={{ fontSize: 11, fontWeight: 700, color: colors.faint, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 8 }}>About</p>
          <p style={{ fontSize: 13.5, color: colors.mid, lineHeight: 1.7 }}>{vendor.about}</p>
        </div>
      )}

      {vendor.onboarding_data && (
        <div style={{ background: '#F7F5F2', borderRadius: 10, padding: '14px 16px', marginBottom: 20 }}>
          <p style={{ fontSize: 11, fontWeight: 700, color: colors.faint, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 10 }}>Onboarding answers</p>
          {vendor.onboarding_data.standout?.length > 0 && (
            <div>
              <p style={{ fontSize: 12, fontWeight: 600, color: colors.dark, marginBottom: 6 }}>How they plan to stand out:</p>
              {vendor.onboarding_data.standout.map((s, i) => (
                <p key={i} style={{ fontSize: 12.5, color: colors.mid, marginBottom: 3 }}>✓ {s.replace(/_/g,' ')}</p>
              ))}
            </div>
          )}
        </div>
      ...
      <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
        <Button variant="secondary" onClick={onClose}>Close</Button>
        {vendor.verified ? (
          <button onClick={() => { onRevoke(vendor.id); onClose(); }} style={{ background: colors.redLight, color: colors.red, border: 'none', borderRadius: 10, padding: '10px 20px', fontSize: 13.5, fontWeight: 700, cursor: 'pointer', fontFamily: font.body }}>
            Revoke verification
          </button>
        ) : (
          <button onClick={() => { onVerify(vendor.id); onClose(); }} style={{ background: colors.green, color: '#fff', border: 'none', borderRadius: 10, padding: '10px 20px', fontSize: 13.5, fontWeight: 700, cursor: 'pointer', fontFamily: font.body }}>
            ✓ Verify vendor
          </button>
        )}
      </div>
    ...
    <div style={{ fontFamily: font.body, background: '#F7F5F2', minHeight: '100vh', paddingBottom: 80 }}>
      {reviewingVendor && (
        <VendorReviewModal
          ...
          onVerify={id => { toggleVerify(id, false); }}
          onRevoke={id => { toggleVerify(id, true); }}
        />
      )}
      <div style={{ background: '#fff', borderBottom: '1px solid #eee', padding: '28px 40px 24px' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '28px 40px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          <div className="card" style={{ padding: '22px 24px' }}>
            ...
            <p style={{ fontSize: 12.5, color: colors.muted, marginBottom: 16 }}>Click Review to inspect a vendor profile before approving</p>
            {vendors.length === 0 ? <p style={{ fontSize: 13, color: colors.muted }}>No vendors yet</p> : vendors.map((v, i) => (
              <div key={v.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingBottom: i<vendors.length-1?14:0, marginBottom: i<vendors.length-1?14:0, borderBottom: i<vendors.length-1?'1px solid #f5f5f5':'none' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <button onClick={() => toggleVerify(v.id, v.verified)} style={{ background: v.verified ? colors.redLight : colors.greenLight, color: v.verified ? colors.red : colors.green, border: 'none', borderRadius: 8, padding: '5px 12px', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: font.body }}>
