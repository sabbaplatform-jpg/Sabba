...
function LevelBadge({ level }) {
  ...
  return (
    <span style={{ fontSize:10, fontWeight:700, color:LEVEL_COLORS[l],
      background:LEVEL_COLORS[l]+'18', borderRadius:5, padding:'2px 6px', letterSpacing:'0.04em' }}>
    ...
    <div style={{ marginBottom:12 }}>
      {!imageUrl ? (
        <div>
          ...
          <button type="button" onClick={()=>fileRef.current?.click()} disabled={uploading}
            style={{ background:'#F7F5F2', border:'1.5px dashed #ddd', borderRadius:10,
              ...
              fontFamily:font.body, cursor:uploading?'default':'pointer', fontWeight:600,
        ...
        <div style={{position:'relative'}}>
          <img src={imageUrl} alt="preview" style={{width:'100%',maxHeight:240,objectFit:'cover',borderRadius:10,display:'block'}}/>
        ...
        <div>
          <div style={{display:'flex',gap:12,alignItems:'flex-start',marginBottom:12}}>
            ...
            <textarea ref={textRef} value={content} onChange={e=>{setContent(e.target.value);setError('');}}
              placeholder="Share your adventure story, tips, or photos…"
              style={{flex:1,border:'none',outline:'none',resize:'none',fontSize:14.5,
                color:colors.dark,fontFamily:font.body,lineHeight:1.6,minHeight:80,background:'transparent'}}/>
          </div>
          <ImageUploader imageUrl={imageUrl} setImageUrl={setImageUrl}/>
          <div style={{display:'flex',gap:8,marginBottom:12,alignItems:'center'}}>
            <span style={{fontSize:12,color:colors.muted,fontWeight:600}}>Visible to:</span>
            {[{v:'global',label:'🌍 Everyone'},{v:'company',label:'🏢 My company'}].map(opt=>(
          ...
          <div style={{display:'flex',justifyContent:'flex-end',gap:8}}>
            <button onClick={()=>{setOpen(false);setContent('');setImageUrl('');setError('');}}
              style={{background:'none',border:'1px solid #eee',borderRadius:10,padding:'8px 18px',
                fontSize:13,cursor:'pointer',fontFamily:font.body,color:colors.mid,fontWeight:600}}>
              Cancel
            </button>
            <button onClick={handlePost} disabled={posting||!content.trim()}
              style={{background:posting||!content.trim()?'#eee':colors.orange,
                color:posting||!content.trim()?colors.muted:'#fff',border:'none',
                borderRadius:10,padding:'8px 20px',fontSize:13,fontWeight:700,
                cursor:posting||!content.trim()?'default':'pointer',fontFamily:font.body}}>
...
function Comments({ postId, count, profile }) {
  ...
  return (
    <div>
      <button onClick={toggle} style={{background:'none',border:'none',color:colors.muted,
        fontSize:13,cursor:'pointer',fontFamily:font.body,fontWeight:600,padding:0,
          ...
          <div style={{display:'flex',gap:10,alignItems:'flex-end',marginTop:8}}>
            ...
            <div style={{flex:1,display:'flex',gap:8}}>
              <input value={text} onChange={e=>setText(e.target.value)}
                ...
                style={{flex:1,border:'1.5px solid #eee',borderRadius:22,padding:'8px 16px',
                  fontSize:13.5,color:colors.dark,fontFamily:font.body,outline:'none'}}/>
              <button onClick={submit} disabled={posting||!text.trim()}
                style={{background:posting||!text.trim()?'#eee':colors.orange,
                  color:posting||!text.trim()?colors.muted:'#fff',border:'none',
...
function PostCard({ post, myProfile, onDelete, navigate }) {
  ...
  const toggleLike = async () => {
    ...
    try {
      ...
      setLiked(data.liked); setLikesCount(c=>data.liked?c+1:c-1);
    } finally { setLiking(false); }
  };

  return (
    <div style={{background:'#fff',border:'1px solid #eee',borderRadius:16,marginBottom:16,overflow:'hidden'}}>
      {post.image_url && (
        ...
        <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:12}}>
          <div style={{display:'flex',alignItems:'center',gap:10,cursor:'pointer'}}
            ...
            <div>
              ...
              <div style={{display:'flex',alignItems:'center',gap:6,marginTop:2,flexWrap:'wrap'}}>
                {post.author_job_title && <span style={{fontSize:12,color:colors.muted}}>{post.author_job_title}</span>}
                {post.author_job_title && <span style={{color:colors.faint}}>·</span>}
                ...
                {post.visibility==='company' && (
                  <span style={{fontSize:10,background:'#F7F5F2',color:colors.muted,borderRadius:4,padding:'1px 6px',fontWeight:600}}>🏢</span>
                )}
              </div>
            </div>
          </div>
          {isOwn && (
            <button onClick={()=>onDelete(post.id)}
              style={{background:'none',border:'none',color:colors.faint,cursor:'pointer',fontSize:13,fontFamily:font.body,padding:'2px 6px'}}>✕</button>
          )}
        </div>
        <p style={{fontSize:14.5,color:colors.mid,lineHeight:1.7,marginBottom:16,whiteSpace:'pre-wrap'}}>{post.content}</p>
        <div style={{display:'flex',alignItems:'center',gap:16,paddingTop:12,borderTop:'1px solid #F7F5F2'}}>
          <button onClick={toggleLike} disabled={liking}
            style={{background:'none',border:'none',cursor:'pointer',display:'flex',
              alignItems:'center',gap:6,fontFamily:font.body,
              color:liked?colors.orange:colors.muted,fontSize:13,fontWeight:liked?700:500,padding:0}}>
...
function MatchedTravellers({ matches, navigate }) {
  ...
  return (
    <div style={{background:'linear-gradient(135deg,#1A2E44,#243d58)',borderRadius:16,padding:'20px 24px',marginBottom:24}}>
      <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4}}>
        <span style={{fontSize:18}}>✈️</span>
        <p style={{fontSize:13,fontWeight:700,color:'#fff',textTransform:'uppercase',letterSpacing:'0.08em'}}>Your travel matches</p>
      </div>
      <p style={{fontSize:12.5,color:'rgba(255,255,255,0.5)',marginBottom:16}}>People heading to the same destination around your dates</p>
      <div style={{display:'flex',gap:12,overflowX:'auto',paddingBottom:4}}>
        {matches.map((m,i)=>(
          <div key={i} style={{background:'rgba(255,255,255,0.08)',borderRadius:14,padding:'14px 16px',
            minWidth:200,flexShrink:0,cursor:'pointer',border:'1px solid rgba(255,255,255,0.1)'}}
            ...
            <div style={{background:'rgba(255,255,255,0.06)',borderRadius:8,padding:'8px 10px',marginBottom:8}}>
              <p style={{fontSize:11.5,color:'rgba(255,255,255,0.7)',fontWeight:600}}>📍 {m.destination}</p>
            ...
            <div style={{display:'flex',gap:6}}>
              <button onClick={e=>{e.stopPropagation();navigate(`/messages/new?to=${m.matched_user_id}&name=${encodeURIComponent(m.matched_name)}`);}}
                style={{flex:1,background:colors.orange,color:'#fff',border:'none',borderRadius:8,
                  padding:'6px',fontSize:11.5,fontWeight:700,cursor:'pointer',fontFamily:font.body}}>
                💬 Message
              </button>
              <button onClick={e=>{e.stopPropagation();m.matched_user_id&&navigate(`/community/profile/${m.matched_user_id}`);}}
                style={{flex:1,background:'rgba(255,255,255,0.1)',color:'#fff',border:'none',borderRadius:8,
                  padding:'6px',fontSize:11.5,fontWeight:700,cursor:'pointer',fontFamily:font.body}}>
                Profile
              </button>
            </div>
...
// ── Platinum Sidebar ───────────────────────────────────────
// slots: [1,2] for left, [3,4] for right
function PlatinumSidebar({ navigate, slots }) {
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:16}}>
      <p style={{fontSize:9.5,fontWeight:700,color:colors.faint,textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:0}}>
        ⭐ Platinum
      </p>
      {filtered.map(l=>(
        <div key={l.id} onClick={()=>navigate(`/package/${l.package_id}`)}
          style={{background:'#fff',border:'1.5px solid #F0C060',borderRadius:16,overflow:'hidden',
            cursor:'pointer',boxShadow:'0 2px 12px rgba(201,136,42,0.12)',transition:'all 0.15s'}}
          ...
          {/* Large image */}
          <div style={{height:160,background:'linear-gradient(135deg,#C9882A,#e8a84a)',position:'relative',overflow:'hidden'}}>
            {l.image_url
              ? <img src={l.image_url} alt={l.package_title} style={{width:'100%',height:'100%',objectFit:'cover'}}/>
              : <div style={{width:'100%',height:'100%',display:'flex',alignItems:'center',justifyContent:'center',fontSize:48}}>{l.emoji||'🌍'}</div>
            }
            {/* Gradient overlay for legibility */}
            <div style={{position:'absolute',inset:0,background:'linear-gradient(to top,rgba(0,0,0,0.5) 0%,transparent 60%)'}}/>
            <div style={{position:'absolute',top:8,left:8,background:GOLD,borderRadius:5,padding:'2px 8px'}}>
              <span style={{fontSize:9,fontWeight:800,color:'#fff',letterSpacing:'0.06em'}}>⭐ PLATINUM</span>
            </div>
            {/* Title on image */}
            <div style={{position:'absolute',bottom:10,left:10,right:10}}>
              <p style={{fontSize:13,fontWeight:700,color:'#fff',lineHeight:1.3,textShadow:'0 1px 4px rgba(0,0,0,0.5)'}}>{l.package_title}</p>
            </div>
          </div>
          {/* Details */}
          <div style={{padding:'12px 14px'}}>
            <p style={{fontSize:11.5,color:colors.muted,marginBottom:8}}>{l.destination} · {l.duration}</p>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-end'}}>
              <div>
                <p style={{fontSize:16,fontWeight:700,color:GOLD}}>£{Number(l.price_gbp).toLocaleString()}</p>
                <p style={{fontSize:10.5,color:colors.faint}}>from £{Math.ceil(l.price_gbp/12)}/mo</p>
              </div>
              <span style={{fontSize:10,fontWeight:700,color:GOLD,background:'#FDF3E3',borderRadius:5,padding:'3px 8px'}}>
                Book now →
              </span>
            </div>
          </div>
...
export function CommunityFeed() {
  ...
  useEffect(() => {
    ...
    }).catch(err => console.error('Community load:', err))
  ...
  const loadMore = async () => {
    setLoadingMore(true);
    const next = page + 1;
    const { data } = await api.get(`/community/feed?page=${next}`);
    setPosts(prev => [...prev, ...data]);
    setPage(next); setHasMore(data.length === 20);
      ...
      <div style={{maxWidth:1200,margin:'0 auto',padding:'32px 20px'}}>
        {[1,2,3].map(i=>(
          <div key={i} style={{background:'#fff',borderRadius:16,marginBottom:16,overflow:'hidden'}}>
            <div style={{height:200,background:'#f0ede9'}}/>
            <div style={{padding:'16px 20px'}}>
              <div style={{height:14,background:'#f0ede9',borderRadius:6,width:'40%',marginBottom:8}}/>
              <div style={{height:12,background:'#f0ede9',borderRadius:6,width:'65%'}}/>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{background:'linear-gradient(135deg,#1A2E44 0%,#243d58 100%)',padding:'28px 40px 24px'}}>
        <div style={{maxWidth:1200,margin:'0 auto'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:20}}>
            <div>
              <p style={{fontSize:10.5,fontWeight:700,color:'rgba(255,255,255,0.45)',textTransform:'uppercase',letterSpacing:'0.12em',marginBottom:6}}>
                Sabba Community
              </p>
              <h1 style={{fontFamily:font.display,fontSize:32,color:'#fff',fontWeight:700,fontStyle:'italic',marginBottom:6}}>
                Adventure Stories
              </h1>
            ...
            {profile && (
              <div style={{display:'flex',alignItems:'center',gap:8,cursor:'pointer',
                background:'rgba(255,255,255,0.1)',borderRadius:22,padding:'8px 16px 8px 10px',
                ...
                <div>
                  <p style={{fontSize:13,fontWeight:700,color:'#fff',lineHeight:1,marginBottom:3}}>{profile.full_name?.split(' ')[0]}</p>
          ...
          <div style={{display:'grid',gridTemplateColumns:window.innerWidth<768?'1fr':'1fr 1fr',gap:12}}>
            <div style={{background:'rgba(255,255,255,0.07)',borderRadius:14,padding:'16px 18px',border:'1px solid rgba(255,255,255,0.1)'}}>
              ...
              {[['📝 Post with photo','20 pts'],['✏️ Text post','10 pts'],['💬 Comment','5 pts'],['❤️ Like received','2 pts'],['✈️ Message a match','30 pts']].map(([a,p])=>(
                <div key={a} style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:5}}>
                  <span style={{fontSize:12,color:'rgba(255,255,255,0.6)'}}>{a}</span>
                  <span style={{fontSize:12,fontWeight:700,color:'#f5a66d'}}>{p}</span>
                </div>
              ))}
            </div>
            <div style={{background:'rgba(255,255,255,0.07)',borderRadius:14,padding:'16px 18px',border:'1px solid rgba(255,255,255,0.1)'}}>
              <p style={{fontSize:11,fontWeight:700,color:'#f5a66d',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:10}}>📋 Community Rules</p>
              {['Share genuine adventure stories and travel tips','Be respectful — this is a professional community','No spam, self-promotion or offensive content','Daily points cap applies — scales with your level','Violations may result in account suspension'].map((r,i)=>(
                <div key={i} style={{display:'flex',gap:8,alignItems:'flex-start',marginBottom:6}}>
                  <span style={{fontSize:10,color:'rgba(255,255,255,0.3)',marginTop:2,flexShrink:0}}>{i+1}.</span>
        ...
        <div>
          ...
          {posts.length === 0 ? (
            <div style={{background:'#fff',borderRadius:16,padding:48,textAlign:'center',border:'1px solid #eee'}}>
              ...
              <p style={{fontSize:16,fontWeight:700,color:colors.dark,marginBottom:8}}>Be the first to share your adventure</p>
              <p style={{fontSize:13.5,color:colors.muted,lineHeight:1.6}}>Share your travel stories, tips and photos with the Sabba community.</p>
            </div>
          ) : (
            <>
              {posts.map(post=>(
                <PostCard key={post.id} post={post} myProfile={profile} onDelete={handleDelete} navigate={navigate}/>
              ))}
              {hasMore && (
...
export function CommunityProfile() {
  ...
  useEffect(() => {
    ...
    const isOwn = id === 'me' || (user && id === user.id);
    const endpoint = isOwn ? '/community/profile/me' : `/community/profile/${id}`;
    Promise.all([
      api.get(endpoint),
      ...
      if (userId) setPosts((f.data||[]).filter(post=>post.user_id===userId));
    }).catch(err=>{ console.error('Profile load error:', err); setLoading(false); })
      .finally(()=>setLoading(false));
  }, [id, authLoading, user]);
  ...
  if (loading || authLoading) return (
    <div style={{display:'flex',alignItems:'center',justifyContent:'center',height:'60vh'}}><Spinner/></div>
  );
  if (!profile) return (
    <div style={{padding:60,textAlign:'center',color:colors.muted,fontFamily:font.body}}>Profile not found</div>
  );
    ...
    <div style={{fontFamily:font.body,background:'#F7F5F2',minHeight:'100vh',paddingBottom:80}}>
      <div style={{background:'#fff',borderBottom:'1px solid #eee',padding:'12px 40px'}}>
        <button onClick={()=>navigate('/community')}
          style={{background:'none',border:'none',color:colors.orange,fontWeight:700,
            fontSize:13,cursor:'pointer',fontFamily:font.body,display:'flex',alignItems:'center',gap:6}}>
          ← Community
        </button>
      </div>
      ...
      <div style={{maxWidth:680,margin:'0 auto',padding:'24px 20px'}}>
        ...
        {/* Profile card */}
        <div style={{background:'#fff',border:'1px solid #eee',borderRadius:20,overflow:'hidden',marginBottom:24}}>
          {/* Banner */}
          <div style={{height:100,background:`linear-gradient(135deg,${LEVEL_COLORS[level]}33,${LEVEL_COLORS[level]}11)`,position:'relative'}}>
          ...
          <div style={{padding:'36px 24px 24px'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:12}}>
              <div>
                <h2 style={{fontSize:22,fontWeight:700,color:colors.dark,marginBottom:4}}>{profile.full_name}</h2>
                <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
                  <LevelBadge level={level}/>
                  {profile.job_title && <span style={{fontSize:12.5,color:colors.muted}}>{profile.job_title}</span>}
                  {profile.job_title && <span style={{color:colors.faint}}>·</span>}
                  <span style={{fontSize:12.5,color:colors.muted}}>{profile.company_name}</span>
                  {profile.adventure_type && (
                    <span style={{fontSize:12,color:colors.faint,textTransform:'capitalize',background:'#F7F5F2',borderRadius:5,padding:'2px 8px'}}>
                      {profile.adventure_type}
                    </span>
                  )}
                </div>
              ...
              {isMe ? (
                <button onClick={()=>setEditing(e=>!e)}
                  style={{background:'#F7F5F2',border:'none',borderRadius:10,padding:'7px 14px',
                    fontSize:12.5,fontWeight:700,cursor:'pointer',color:colors.mid,fontFamily:font.body}}>
                  {editing?'Cancel':'Edit profile'}
                </button>
              ) : (
                <button onClick={()=>navigate(`/messages/new?to=${profile.user_id}&name=${encodeURIComponent(profile.full_name)}`)}
                  style={{background:colors.orange,color:'#fff',border:'none',borderRadius:10,
                    padding:'8px 16px',fontSize:13,fontWeight:700,cursor:'pointer',fontFamily:font.body}}>
                  💬 Message
                </button>
              )}
            </div>
            ...
            <div style={{marginBottom:16}}>
              <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
                <span style={{fontSize:11,color:colors.faint,fontWeight:600}}>Level {level} · {LEVEL_NAMES[level]}</span>
                {level<5 && <span style={{fontSize:11,color:colors.faint}}>{(profile.sabba_points||0).toLocaleString()} / {LEVEL_THRESHOLDS[level+1]?.toLocaleString()} pts</span>}
              </div>
              <div style={{height:4,background:'#eee',borderRadius:2,overflow:'hidden'}}>
                <div style={{height:'100%',borderRadius:2,background:LEVEL_COLORS[level],
            ...
            {isMe && editing ? (
              ...
              <div style={{background:'#F7F5F2',borderRadius:10,padding:'14px 16px',cursor:'pointer',
                border:'1.5px dashed #ddd',textAlign:'center'}}
                onClick={()=>setEditing(true)}>
                <p style={{fontSize:13.5,color:colors.muted,fontStyle:'italic'}}>
                  ✏️ Click "Edit profile" to add a bio and tell the community about yourself
                </p>
              </div>
            ) : (
              <p style={{fontSize:14,color:profile.bio?colors.mid:colors.faint,lineHeight:1.7,fontStyle:profile.bio?'normal':'italic'}}>
        ...
        {/* Posts */}
        <p style={{fontSize:11,fontWeight:700,color:colors.faint,textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:14}}>
          Posts · {posts.length}
        </p>
        {posts.length===0 ? (
          <div style={{background:'#fff',borderRadius:16,padding:32,textAlign:'center',border:'1px solid #eee'}}>
            <p style={{fontSize:14,color:colors.muted}}>{isMe?'Share your first adventure story!':'No posts yet.'}</p>
          </div>
        ) : posts.map(post=>(
